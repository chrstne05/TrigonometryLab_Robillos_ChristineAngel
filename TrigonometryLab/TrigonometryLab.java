...
public class TrigonometryLab {
    ...
    public static void main(String[] args) {
        ...
        double normalized = quadrantAngle % 360;
        ...
        System.out.print("Enter your choice: ");
        char triangleChoice = input.next().charAt(0);
        ...
        System.out.println("sin²θ + cos²θ = 1");
            ...
            System.out.println("1 + tan²θ = sec²θ");
                ...
                double right2 = secValue * secValue;
        ...
        // Frequency represents the number of cycles per unit of time, so it must be greater than zero.
            ...
            double phaseRadians = toRadians(phaseShift);
            ...
            double nextPeakAngle =
                    (PI / 2)
                    + (2 * PI * peakNumber);

            // Solve for the time when the next peak occurs.
            double nextPeakTime =
                    (nextPeakAngle - phaseRadians)
                    / (2 * PI * frequency);

            // If the calculated peak is already at the current time, find the following peak.
            if (nextPeakTime <= time + tolerance) {

                nextPeakTime = nextPeakTime + (1 / frequency);
            }

            System.out.println(
                "Next peak time = " + nextPeakTime);
        }    
